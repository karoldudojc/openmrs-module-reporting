...
public class DrugOrdersForPatientDataEvaluator1_10 implements PatientDataEvaluator {
    ...
    public EvaluatedPatientData evaluate(PatientDataDefinition definition, EvaluationContext context) throws EvaluationException {
        ...
        if (def.getCompletedOnOrBefore() != null) {
            hql.append("and		 (autoExpireDate is not null and autoExpireDate <= :completedOnOrBefore)");
            m.put("completedOnOrBefore", DateUtil.getEndOfDayIfTimeExcluded(def.getCompletedOnOrBefore()));
        }

        if (def.getCompletedOnOrAfter() != null) {
            hql.append("and	     (autoExpireDate is not null and autoExpireDate >= :completedOnOrAfter)");
